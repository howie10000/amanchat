...
function reset(ship='brig'){
 ...
 service=require('../server-node/crew-sea')({rules:{...S,sector:(seed,x,y)=>x===0&&y===0?[land]:[]},isStaff:u=>u==='tester',getUser:()=>user,save:(_,p)=>user.sea=p,pay:(_,n)=>user.money-=n,seed:()=>123});
}
...
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,'http://127.0.0.1');
 if(req.method==='POST'&&url.pathname==='/action'){if(req.headers.origin&&req.headers.origin!=='http://127.0.0.1:18445')throw Error('Local fixture only');let text='';for await(const chunk of req){text+=chunk;if(text.length>8192)throw Error('Too large');}const m=JSON.parse(text||'{}');
  ...
  else if(m.action==='fixture_great'){const v=service.raw('tester');v.x=5000;v.y=1000;v.speed=0;service.tick();service.handle('tester',{action:'staff_summon',kind:'great'});}
 ...
 if(url.pathname==='/title'){res.setHeader('content-type','text/html');res.end(fs.readFileSync(path.join(root,'index.html'),'utf8').split('<!-- GAME SCREEN -->')[0]+'<script src="/js/vendor/three.min.js"></script><script src="/js/shared/sea.js"></script><script>const canvas=document.getElementById("titleBg"),state={area:"login"},keys={};</script><script src="/assets/dark-sea/blender-meshes.js?v=blender-1"></script><script src="/assets/dark-sea/legendary-models.js?v=legendary-2"></script><script src="/assets/dark-sea/blender-animations.js?v=1"></script><script src="/js/sea-animation.js?v=1"></script><script src="/js/sea3d.js"></script><script src="/js/sea-title.js"></script></body></html>');return;}
 const file=url.pathname==='/'?'/docs/dark-sea-playtest.html':url.pathname;if(!['/assets/dark-sea/title-crest.png','/assets/dark-sea/title-wordmark.png','/assets/dark-sea/title-scroll.png'].includes(file)&&file!=='/assets/dark-sea/blender-animations.js'&&file!=='/assets/dark-sea/blender-meshes.js'&&file!=='/assets/dark-sea/legendary-models.js'&&!/^\/(js\/[^.][\w/.-]*\.js|style\.css|docs\/dark-sea-playtest\.html)$/.test(file))throw Error('Not available');const target=path.resolve(root,'.'+file);if(!target.startsWith(root+path.sep))throw Error('Not available');res.setHeader('content-type',file.endsWith('.png')?'image/png':file.endsWith('.js')?'text/javascript':file.endsWith('.css')?'text/css':'text/html');res.end(fs.readFileSync(target));
