...
(function(root){'use strict';
 const TAU=Math.PI*2, WIDTH=22, COUNT=240;
 ...
 function generate(random=Math.random,options={}){
  ...
  const length=distances.at(-1),points=[];rng();let cursor=0;
  for(let i=0;i<COUNT;i++){const distance=i/COUNT*length;while(distances[cursor+1]<distance)cursor++;const a=dense[cursor],b=dense[(cursor+1)%dense.length],t=(distance-distances[cursor])/(distances[cursor+1]-distances[cursor]);
   // Asphalt sits on the surrounding ground with slight corner camber.
   points.push({x:a.x+(b.x-a.x)*t,z:a.z+(b.z-a.z)*t,y:.2});
  }
  for(let i=0;i<COUNT;i++){const a=points[(i+COUNT-2)%COUNT],p=points[i],b=points[(i+2)%COUNT],incoming=Math.atan2(p.x-a.x,p.z-a.z),outgoing=Math.atan2(b.x-p.x,b.z-p.z);p.bank=Math.max(-.012,Math.min(.012,Math.atan2(Math.sin(outgoing-incoming),Math.cos(outgoing-incoming))*.7));p.boost=i>=4&&i<=9;p.ramp=false;}
  const segments=points.map((p,i)=>{const q=points[(i+1)%COUNT],dx=q.x-p.x,dz=q.z-p.z,len=Math.hypot(dx,dz);return{p,q,dx,dz,len};});
  return{width:WIDTH,seed,points,segments,length,name:layout.name,theme:Math.floor(rng()*3),mode,difficulty};
 }
 function endless(random,difficulty){
  const track={width:WIDTH,seed:(random()*0xffffffff)>>>0,points:[],segments:[],length:0,name:'Endless Highway',theme:0,mode:'endless',difficulty,open:true,nextId:0,heading:0,rngState:0};track.rngState=track.seed;extend(track,300);return track;
 }
 function extend(track,count=60){
  ...
  for(let i=0;i<count;i++){const id=track.nextId++,prev=track.points.at(-1)||{x:0,z:-5,y:4},strength=track.difficulty==='easy'?.013:track.difficulty==='hard'?.045:.027;
   ...
   track.points.push({x:prev.x+Math.sin(track.heading)*5,z:prev.z+Math.cos(track.heading)*5,y:.2,bank:Math.max(-.012,Math.min(.012,(track.turn||0)*.3)),boost:id%90>=20&&id%90<26,ramp:false,id});
  }
 ...
 function step(car,track,input,dt){
  ...
  if(car.grounded&&hit.distance>WIDTH/2-.9&&hit.distance<WIDTH/2+3&&Math.abs(car.y-hit.y-.55)<1){const s=track.segments[hit.index],px=s.p.x+s.dx*hit.t,pz=s.p.z+s.dz*hit.t,nx=-s.dz/s.len,nz=s.dx/s.len,side=Math.sign((car.x-px)*nx+(car.z-pz)*nz);car.x=px+nx*side*(WIDTH/2-1);car.z=pz+nz*side*(WIDTH/2-1);car.speed*=.7;car.yaw=Math.atan2(s.dx,s.dz);hit=nearest(track,car.x,car.z);}
