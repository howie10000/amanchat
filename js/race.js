...
(function(root){'use strict';
 ...
 function generate(random=Math.random){
  ...
  for(let i=0;i<COUNT;i++){const distance=i/COUNT*length;while(distances[cursor+1]<distance)cursor++;const a=dense[cursor],b=dense[(cursor+1)%dense.length],t=(distance-distances[cursor])/(distances[cursor+1]-distances[cursor]),f=i/COUNT;
   ...
   points.push({x:a.x+(b.x-a.x)*t,z:a.z+(b.z-a.z)*t,y:4+bridge+2*Math.sin(f*TAU*2+phase)});
  }
 ...
 function sample(track,distance){const index=((distance/track.length*COUNT)%COUNT+COUNT)%COUNT,i=Math.floor(index),t=index-i,s=track.segments[i];return{x:s.p.x+s.dx*t,z:s.p.z+s.dz*t,y:s.p.y+(s.q.y-s.p.y)*t,yaw:Math.atan2(s.dx,s.dz),pitch:-Math.atan2(s.q.y-s.p.y,s.len),index:i};}
 ...
 api.start=function(){
  ...
  function frame(now){if(!api.active)return;raf=requestAnimationFrame(frame);if(document.hidden){last=0;return;}const dt=last?Math.min((now-last)/1000,.1):0;last=now;acc+=dt;
   ...
   const under=nearest(track,car.x,car.z),segment=track.segments[under.index];art.poseCar(carModel,{...car,y:car.y-.55,pitch:car.grounded?-Math.atan2(segment.q.y-segment.p.y,segment.len):-.06},elapsed,car.speed);
   ...
   timeText.textContent=Math.floor(elapsed/60)+':'+(elapsed%60).toFixed(3).padStart(6,'0');speedText.textContent=Math.round(Math.abs(car.speed)*3.6)+' km/h';progressText.textContent=finished?'FINISHED · Try another circuit!':!started?'Press W or Gas to start':`Checkpoint ${checkpoint} / 8 · Recovery +3s`;
