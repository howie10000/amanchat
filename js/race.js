...
(function(root){'use strict';
 ...
 api.start=function(){
  ...
  function road(){
   ...
   const m=mapStatic.getContext('2d');m.fillStyle='#102637e6';m.fillRect(0,0,180,130);m.strokeStyle='#a6bac3';m.lineWidth=4;m.beginPath();track.points.forEach((p,i)=>i?m.lineTo(p.x*mapScale+mapX,p.z*mapScale+mapZ):m.moveTo(p.x*mapScale+mapX,p.z*mapScale+mapZ));if(!track.open)m.closePath();m.stroke();
  }
  function reset(fresh){if(fresh||!track){track=generate(Math.random,{mode:host.querySelector('.race-mode').value,difficulty:host.querySelector('.race-difficulty').value});road();host.querySelector('h2').textContent=track.name;}car=spawn(track);checkpoint=0;elapsed=0;started=false;finished=false;acc=0;progressText.textContent='Checkpoint 0 / 8';camera.position.set(car.x-Math.sin(car.yaw)*12,car.y+7,car.z-Math.cos(car.yaw)*12);}
  ...
  host.querySelector('.race-mode').onchange=host.querySelector('.race-difficulty').onchange=()=>reset(true);host.querySelector('[data-race="exit"]').onclick=stop;host.querySelector('[data-race="retry"]').onclick=()=>reset(false);host.querySelector('[data-race="new"]').onclick=()=>reset(true);
  ...
  function frame(now){if(!api.active)return;raf=requestAnimationFrame(frame);if(document.hidden){last=0;return;}const dt=last?Math.min((now-last)/1000,.1):0;last=now;acc+=dt;
   ...
   timeText.textContent=Math.floor(elapsed/60)+':'+(elapsed%60).toFixed(3).padStart(6,'0');speedText.textContent=Math.round(Math.abs(car.speed)*3.6)+' km/h'+(car.boost?' · BOOST':'');progressText.textContent=finished?'FINISHED · Try another circuit!':!started?'Press W or Gas to start':track.open?`Endless · ${(track.points[under.index].id*5/1000).toFixed(2)} km · ${checkpoint} checkpoints`:`Checkpoint ${checkpoint} / 8 · Recovery +3s`;
