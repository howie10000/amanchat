...
function drawShipwrightDetail(b){
 ...
 ctx.fillStyle='#cab78f';ctx.font='11px Georgia';ctx.fillText('VESSELS · CREWS · REFITS',b.x+b.w/2,b.y+119);for(const x of [b.x+21,b.x+b.w-53]){ctx.fillStyle='#152c35';ctx.fillRect(x,b.y+39,34,34);ctx.fillStyle='#eddca4';ctx.fillRect(x+3,b.y+42,28,28);ctx.strokeStyle='#856d48';ctx.beginPath();ctx.moveTo(x+17,b.y+42);ctx.lineTo(x+17,b.y+70);ctx.moveTo(x+3,b.y+56);ctx.lineTo(x+31,b.y+56);ctx.stroke();}ctx.restore(); ctx.save();const bx=b.x+22,by=b.y+b.h-46;ctx.fillStyle='#112b32';ctx.fillRect(bx-7,by-22,65,66);ctx.strokeStyle='#a78654';ctx.lineWidth=4;ctx.strokeRect(bx-7,by-22,65,66);ctx.fillStyle='#704a30';ctx.fillRect(bx+4,by+26,7,19);ctx.fillRect(bx+39,by+26,7,19);ctx.beginPath();ctx.moveTo(bx-1,by+5);ctx.quadraticCurveTo(bx+27,by+42,bx+53,by+5);ctx.lineTo(bx+48,by-3);ctx.lineTo(bx+3,by-3);ctx.closePath();ctx.fill();ctx.strokeStyle='#d8b880';ctx.lineWidth=2;for(let x=7;x<49;x+=9){ctx.beginPath();ctx.moveTo(bx+x,by);ctx.lineTo(bx+x,by+20-Math.abs(x-27)*.45);ctx.stroke();}ctx.restore();
