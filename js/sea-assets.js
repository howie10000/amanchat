...
function start(){if(!login||login.classList.contains('hidden')||document.hidden)return;if(!title)title=window.loadRacingAssets().then(()=>{if(login.classList.contains('hidden')){title=null;window.gameRace?.preload();return;}return script('js/race-title.js?v=league-3');}).catch(()=>{title=null;});}
