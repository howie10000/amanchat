...
].map(([key,url])=>window[key]?Promise.resolve():script(url))).catch(e=>{assets=null;throw e;});};
function start(){if(!login||login.classList.contains('hidden')||document.hidden)return;window.loadSeaAssets().then(()=>{
 if(login.classList.contains('hidden')||document.hidden)return;
 if(!title)title=script('js/sea-title.js?v=part2-cold-24').catch(()=>{title=null;});
}).catch(()=>{});}
window.titleBg={start};
// Give HTML, form handlers and the lightweight backdrop a chance to paint first.
requestAnimationFrame(()=>requestAnimationFrame(()=>{
 if(window.requestIdleCallback)window.requestIdleCallback(start,{timeout:2000});else setTimeout(start,100);
}));
