...
window.loadRacingAssets=function(){return racing??=script('js/race-models.js?v=apex-laps-4').catch(e=>{racing=null;throw e;});};
function start(){if(!login||login.classList.contains('hidden')||document.hidden)return;if(!title)title=window.loadRacingAssets().then(()=>{if(login.classList.contains('hidden')){title=null;window.gameRace?.preload();return;}return script('js/race-title.js?v=apex-laps-4');}).catch(()=>{title=null;});}
