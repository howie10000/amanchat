...
function draw(c,name,x,y,size){name=aliases[name]||name;c.save();c.translate(x,y);c.scale(size/100,size/100);c.lineJoin='round';c.lineCap='round';c.lineWidth=5;c.strokeStyle='#48351e';
 ...
 c.restore();}
const urls=new Map();
function url(name){if(!urls.has(name)){const cv=document.createElement('canvas');cv.width=cv.height=128;draw(cv.getContext('2d',{willReadFrequently:true}),name,64,64,110);urls.set(name,cv.toDataURL());}return urls.get(name);}
function html(name,size=36){return `<img alt="${aliases[name]||name}" width="${size}" height="${size}" style="vertical-align:middle;object-fit:contain" src="${url(name)}">`;}
