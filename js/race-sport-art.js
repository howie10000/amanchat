...
(function(root){'use strict';
 ...
 function create(){
  ...
  function circuit(track){
   ...
   const middle=points[Math.floor(n/2)];const gx=points.map(p=>p.x),gz=points.map(p=>p.z);box(group,(Math.min(...gx)+Math.max(...gx))/2,-.62,(Math.min(...gz)+Math.max(...gz))/2,Math.max(...gx)-Math.min(...gx)+4000,1,Math.max(...gz)-Math.min(...gz)+4000,palette.ground);
