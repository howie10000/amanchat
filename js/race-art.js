...
(function(root){'use strict';
 ...
 function create(){
  ...
  function unpack(name,key,data){const id=name+':'+key;if(blenderGeometries.has(id))return blenderGeometries.get(id);const bytes=s=>{const raw=atob(s),a=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)a[i]=raw.charCodeAt(i);return a.buffer;},packed=new Uint16Array(bytes(data.p)),normals=new Int8Array(bytes(data.n)),positions=new Float32Array(packed.length),n=new Float32Array(normals.length);for(let i=0;i<packed.length;i++)positions[i]=data.b[i%3]+packed[i]/65535*(data.b[3+i%3]-data.b[i%3]);for(let i=0;i<n.length;i++)n[i]=normals[i]/127;const geo=own(new THREE.BufferGeometry());geo.setAttribute('position',new THREE.BufferAttribute(positions,3));geo.setAttribute('normal',new THREE.BufferAttribute(n,3));geo.setIndex(new THREE.BufferAttribute(new Uint32Array(bytes(data.i)),1));geo.computeBoundingSphere();blenderGeometries.set(id,geo);return geo;}
  function blenderPart(name,overrides={}){const g=new THREE.Group();g.userData.blenderPart=name;for(const [key,data]of Object.entries(root.ApexModels.kits[name])){const settings=root.ApexModels.materials[key],material=overrides[key]||finish('blender:'+key,{...settings,clearcoat:key==='paint'||key==='glass'?1:0,clearcoatRoughness:.12,envMapIntensity:key==='glass'?.65:key==='paint'?1.5:.3,side:key.startsWith('leaf')?THREE.DoubleSide:THREE.FrontSide});if(key==='soil'||key==='rock')material.onBeforeCompile=shader=>{shader.vertexShader=shader.vertexShader.replace('#include <common>','#include <common>\nvarying vec3 landscape;').replace('#include <worldpos_vertex>','#include <worldpos_vertex>\nvec4 lp=vec4(transformed,1.0);\n#ifdef USE_INSTANCING\nlp=instanceMatrix*lp;\n#endif\nlandscape=(modelMatrix*lp).xyz;');shader.fragmentShader=shader.fragmentShader.replace('#include <common>','#include <common>\nvarying vec3 landscape;').replace('#include <color_fragment>','#include <color_fragment>\nfloat layers=sin(landscape.x*.17+sin(landscape.z*.2))*sin(landscape.z*.13+landscape.y*.11);float detail=sin(landscape.x*2.3+landscape.y*1.7)*sin(landscape.z*2.7);diffuseColor.rgb*=.93+layers*.2+detail*.055;');};const m=new THREE.Mesh(unpack(name,key,data),material);m.userData.blender=true;m.castShadow=true;m.receiveShadow=true;g.add(m);}return g;}
  function blenderCar(color,number){const g=new THREE.Group(),paint=finish('blender-paint:'+color,{color,metalness:.72,roughness:.23,clearcoat:1,clearcoatRoughness:.1,envMapIntensity:1.6}),tail=finish('blender-tail:'+number,{color:'#ef2821',emissive:'#e9251b',emissiveIntensity:.6,roughness:.23}),head=finish('blender-head',{color:'#deedff',emissive:'#c9e3ff',emissiveIntensity:1.3,roughness:.18}),body=blenderPart('body',{paint,tail,head});g.add(body);Object.assign(g.userData,{body,wheels:[],steering:[],wheelAngle:0,lastTime:null,lastSpeed:0,brakeMaterial:tail,blender:true});
   for(const side of [-1,1])for(const z of [-1.42,1.38]){const pivot=new THREE.Group();pivot.position.set(side*1.01,-.16,z);g.add(pivot);const wheel=blenderPart('wheel');pivot.add(wheel);g.userData.wheels.push(wheel);if(z>0)g.userData.steering.push(pivot);const caliper=blenderPart('caliper');caliper.position.x=side*.17;pivot.add(caliper);}return g;}
  function car(color='#28485e',number=7){
   if(root.ApexModels)return blenderCar(color,number);
  ...
  function circuit(track){
   ...
   for(let i=0;i<n;i+=6){const p=points[i],norm=normals[i];for(const side of [-1,1]){
    ...
    if(root.ApexModels){const tree=blenderPart('tree');tree.position.set(x,-.1,z);const size=1.25+(i%5)*.12;tree.scale.set(size,size,size);tree.rotation.y=i*1.73;group.add(tree);if(i%12===0){const rock=blenderPart('rock');rock.position.set(x+4,-.05,z+3);rock.scale.set(1.5,.9,1.2);rock.rotation.y=i;group.add(rock);}}
   }}
   for(const obstacle of track.mountains||[]){const m=root.ApexModels?blenderPart('hill'):new THREE.Mesh(mountain,mat('#5e7466'));m.position.set(obstacle.x,obstacle.y,obstacle.z);m.scale.set(obstacle.rx,obstacle.height,obstacle.rz);group.add(m);}
   // Static cubes share geometry and are instanced by material: no hundreds of draw calls.
   group.updateMatrixWorld(true);const batches=new Map(),remove=[];group.traverse(m=>{if(m.isMesh&&(m.geometry===cube||m.geometry===cone||m.geometry===mountain||m.userData.blender)){const key=m.geometry.id+':'+m.material.id;let batch=batches.get(key);if(!batch)batches.set(key,batch={geometry:m.geometry,material:m.material,list:[]});batch.list.push(m.matrixWorld.clone());remove.push(m);}});for(const m of remove)m.removeFromParent();for(const {geometry,material,list}of batches.values()){const mesh=new THREE.InstancedMesh(geometry,material,list.length);list.forEach((matrix,i)=>mesh.setMatrixAt(i,matrix));mesh.instanceMatrix.needsUpdate=true;mesh.castShadow=true;mesh.receiveShadow=true;group.add(mesh);}
   group.userData.ownedGeometries=owned;group.userData.blender=!!root.ApexModels;return group;
  }
