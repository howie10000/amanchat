...
(function(root){'use strict';
 ...
 function create(){
  ...
  function updateLighting(scene,p,time=0){for(const root of scene.children){if(root.visible===false)continue;root.traverse(m=>{const c=m.userData.sceneryCell;if(c)m.visible=Math.hypot(c.x-p.x,c.z-p.z)<900;});}const sun=scene.userData.sun;if(!sun)return;const d=sun.userData.dir||new THREE.Vector3(-.65,.32,.4);sun.position.set(p.x+d.x*120,p.y+Math.max(18,d.y*120),p.z+d.z*120);sun.target.position.set(p.x,p.y,p.z);sun.target.updateMatrixWorld();if(scene.userData.fill){scene.userData.fill.position.set(p.x-d.x*80,p.y+40,p.z-d.z*80);scene.userData.fill.target.position.set(p.x,p.y,p.z);scene.userData.fill.target.updateMatrixWorld();}scene.userData.sky.position.set(p.x,p.y,p.z);scene.userData.sky.material.uniforms.time.value=time;timeUniform.value=time;for(const obj of scene.children)if(obj.visible!==false&&obj.userData&&obj.userData.blender&&obj.userData.night!==scene.userData.night)obj.userData.night=scene.userData.night;}
