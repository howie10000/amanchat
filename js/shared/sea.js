...
function navalEncounter(kind,tier,x,y,id,rng){
 ...
 return {id,kind,name:kind==='kraken'?'Leviathan':'Pirate '+SHIPS[ship].name,ship,x,y,homeX:x,homeY:y,siege:kind==='pirate'&&rng()<.12+tier*.08,mortarTier:tier>=2&&rng()<.3?'heavy':rng()<.55?'light':'medium',a:0,hp,maxHp:hp,tier,attackAt:0,warning:0};
...
function expeditionIsland(e,rng){e.terrain=true;e.hills=Array.from({length:9},()=>{const a=rng()*Math.PI*2,r=e.r*(.1+rng()*.5);return{x:e.x+Math.cos(a)*r,y:e.y+Math.sin(a)*r,height:90+rng()*260,width:e.r*(.12+rng()*.17)};});
 ...
 e.caves=Array.from({length:2+Math.floor(rng()*2)},(_,i)=>{const a=i*Math.PI*2/3+e.phase,x=e.x+Math.cos(a)*e.r*.57,y=e.y+Math.sin(a)*e.r*.57,id=e.id+':cave:'+i,rooms=[{x,y,r:125}];for(let n=1;n<3+e.tier;n++){const prev=rooms[n-1],heading=a+Math.PI+(rng()-.5)*1.2;rooms.push({x:prev.x+Math.cos(heading)*210,y:prev.y+Math.sin(heading)*210,r:125+rng()*45});}for(let n=1;n<rooms.length;n++){const room=rooms[n];e.chests.push({id:id+':cache:'+n,x:room.x+30,y:room.y,tier:Math.min(3,e.tier+1),taken:false,cave:id});}return{id,x,y,rooms};});
...
function landWaypoint(e,p,goal){let target=goal;const f=e?.fort;if(f&&fortWalls(e).some(w=>lineBox(p,goal,w,10))){const inside=Math.abs(p.x-f.x)<f.r-8&&Math.abs(p.y-f.y)<f.r-8;target={x:f.x,y:f.y+f.r+(inside?-40:40)};if(Math.hypot(p.x-target.x,p.y-target.y)<25)target={x:f.x,y:f.y+f.r+(inside?40:-40)};}const gap=Math.hypot(target.x-p.x,target.y-p.y),end={x:p.x+(target.x-p.x)/Math.max(1,gap)*Math.min(85,gap),y:p.y+(target.y-p.y)/Math.max(1,gap)*Math.min(85,gap)},rock=islandProps(e).find(o=>segmentHit(p,end,o,o.r+12)!==null);if(rock){const a=Math.atan2(target.y-p.y,target.x-p.x),cross=(rock.x-p.x)*Math.sin(a)-(rock.y-p.y)*Math.cos(a),side=cross>0?1:-1;target={x:rock.x+Math.cos(a+side*Math.PI/2)*(rock.r+22),y:rock.y+Math.sin(a+side*Math.PI/2)*(rock.r+22)};}return target;}
