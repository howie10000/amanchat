...
(function () {
  ...
  function rebuild(key){
    ...
    if(state.area==='neighborhood'){
      ...
      if(state.area==='interior_dealership'){
        const dealer=avatar({skin:'#d9a77c',shirt:'#164e63',pants:'#172033',hair:'#262025'});dealer.position.set(512*U,0,208*U);dealer.rotation.y=Math.PI;terrain.add(dealer);label(terrain,'THE DEALER',512*U,2.9,208*U);
        Object.entries(CARS).forEach(([id,c],i)=>{const x=(260+i*250)*U,z=390*U;box(terrain,x,.55,z,1.8,.7,3.2,c.color);box(terrain,x,1.1,z,1.4,.6,1.5,'#28465b');for(const a of [-1,1])for(const b of [-1,1])box(terrain,x+a*.95,.35,z+b, .35,.7,.7,'#142132');label(terrain,c.name,x,2.3,z);});
      }
  ...
  function syncActor(id,a,x,y,facing,local=false,carId=''){let entry=actors.get(id),sig=JSON.stringify([a,carId]);if(!entry||entry.sig!==sig){if(entry)dispose(entry.g);const g=avatar(a);if(CARS[carId]){for(const child of [...g.children])child.visible=false;const c=CARS[carId];box(g,0,.55,0,1.8,.7,3.2,c.color);box(g,0,1.1,0,1.4,.6,1.5,'#28465b');for(const x of [-.95,.95])for(const z of [-1,1])box(g,x,.35,z,.35,.7,.7,'#142132');}moving.add(g);entry={g,sig};actors.set(id,entry);}const g=entry.g,walking=Math.hypot(g.position.x-x*U,g.position.z-y*U)>.002;g.position.set(x*U,0,y*U);for(const [i,leg]of (g.userData.legs||[]).entries())leg.rotation.x=walking?Math.sin(Date.now()/100+i*Math.PI)*.35:0;g.rotation.y=local?yaw:({up:0,down:Math.PI,left:Math.PI/2,right:-Math.PI/2}[facing]||0);g.visible=true;if(local)for(const child of g.children)if(child.position.y>=1.4)child.visible=false;const t=Date.now()/1000;if(g.userData.aura)g.userData.aura.rotation.y=t;if(g.userData.pet)g.userData.pet.position.y=Math.sin(t*5)*.04;return g;}
