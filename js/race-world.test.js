...
for(const generation of [1,2,3])for(const mode of ['short','long','endless'])for(const difficulty of ['easy','medium','hard'])for(let seed=1;seed<=10;seed++){
 const t=race.generate(()=>seed/11,{generation,mode,difficulty});for(let k=0;k<(t.open?8:1);k++){if(t.open)race.extend(t);assert(t.mountains.length>5&&t.mountains.length<=26);for(const m of t.mountains)for(const seg of t.segments)assert(world.segmentDistance(m.x,m.z,seg)>m.radius+t.width/2+40);}
