...
(function(root){'use strict';
 ...
 for(let i=0;i<track.segments.length;i+=30){const s=track.segments[i],g=new THREE.Group();g.position.set(s.p.x,s.p.y,s.p.z);g.rotation.y=Math.atan2(s.dx,s.dz);group.add(g);const color=i===0?'#a3ed74':'#54dbe1';for(const x of [-6.8,6.8])box(g,x,3,0,.45,6,.45,color);box(g,0,6,0,14,.45,.45,color);}
 const mid=track.points[Math.floor(COUNT/2)];box(group,track.open?mid.x:0,-5,track.open?mid.z:0,track.open?2600:1000,1,track.open?2600:1000,'#698b79');
