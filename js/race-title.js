...
(function(){'use strict';
 ...
 function init(){
  ...
  art=RaceArt.create();art.configure(renderer);renderer.setPixelRatio(1);track=gameRace.generate(()=>.742019,{generation:3});scene=art.scene(track);scene.add(art.circuit(track));camera=new THREE.PerspectiveCamera(60,1,.1,1400);art.setCamera?.(camera);
 ...
 function frame(stamp){raf=0;if(document.hidden||login.classList.contains('hidden')){stop();return;}if(typing())return;if(!renderer&&!init())return;if(last&&stamp-last<32&&!reduced.matches){raf=requestAnimationFrame(frame);return;}if(last)time+=Math.min(.1,(stamp-last)/1000);last=stamp;const t=reduced.matches?3.6:time;
  ...
  const p=gameRace.sample(track,0),ahead=gameRace.sample(track,42);camera.position.set(p.x-Math.cos(p.yaw)*10,p.y+2.4,p.z+Math.sin(p.yaw)*10);camera.lookAt(ahead.x,ahead.y+1,ahead.z);art.updateLighting(scene,p,t);renderer.render(scene,camera);canvas.classList?.add("ready");frames++;if(!reduced.matches)raf=requestAnimationFrame(frame);
 }
 ...
 function start(){if(typing())return;if(document.hidden||login.classList.contains('hidden')){stop();return;}if(!raf&&!lost){last=0;raf=requestAnimationFrame(frame);}}
