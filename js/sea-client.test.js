...
async function test(){
 ...
 const autoBtn={classList:{toggle(n,on){this[n]=on;},contains(n){return!!this[n];}},setAttribute(k,v){this[k]=v;},dataset:{sea:'auto'}};
 const elements=new Map(),el=id=>{if(!elements.has(id))elements.set(id,{tagName:'DIV',innerHTML:'',classList:{add(){}},appendChild(){},querySelectorAll(){return[];},querySelector:sel=>String(sel).includes('auto')?autoBtn:null});return elements.get(id);};
 ...
 assert(el('hud').innerHTML.includes('seaAutoLed'),'Auto-fire control keeps a status LED');
 voyage.auto=false;await sea.act('input');assert.equal(autoBtn.classList.autoOn,false);assert.equal(autoBtn['aria-pressed'],'false');
 voyage.auto=true;await sea.act('input');assert.equal(autoBtn.classList.autoOn,true,'LED tracks auto-fire on');assert.equal(autoBtn['aria-pressed'],'true');
