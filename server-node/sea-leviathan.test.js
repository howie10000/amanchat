'use strict';
const assert=require('node:assert/strict'),S=require('../js/shared/sea'),attack=require('./sea-leviathan')(S,S.random('random-attack-tests'));
const e={attackCount:0},v={x:100,y:200,a:.4,speed:100,turnVelocity:.1};const names=[];
for(let i=0;i<14;i++){attack(e,v,1000+i*10000);const a=e.attack;names.push(a.type);assert(a.impact-a.start>=2100);assert(a.end>a.impact);assert(a.targets.length<=6);assert(a.targets.every(p=>Number.isFinite(p.x)&&Number.isFinite(p.y)));if(a.grand){assert(a.impact-a.start>=3000);assert(a.targets.length>=3);assert.equal(a.damageScale,1.2);}const saved=JSON.stringify(a.targets);v.x+=10;assert.equal(JSON.stringify(a.targets),saved,'Telegraphs freeze instead of tracking the player');}
assert(new Set(names).size>=4);assert.notDeepEqual(names.slice(0,7),names.slice(7),'Attacks do not cycle in a fixed order');
console.log('PASS seven Leviathan attack patterns, bounded targets, readable grand windups and frozen dodgeable telegraphs');
