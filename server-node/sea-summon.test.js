...
// A second summon succeeds at the exact same server timestamp.
...
r=call('admin','staff_summon',{kind:'great'});const greatId=r.voyage.entities.find(e=>e.kind==='kraken'&&e.id!==levi.id).id,raw=[...sea.raw('admin').sectors.values()].flat().find(e=>e.id===greatId);assert(raw.greatCandidate);assert(!raw.great&&!raw.cinematic);const summonedAt=time;time+=4999;sea.tick();assert(!raw.cinematic,'Normal Leviathan remains for the full five-second lead-in');time++;sea.tick();assert.equal(raw.cinematic.start,summonedAt+5000);assert.equal(call('guest','status').voyage.cinematic.id,greatId);assert(!raw.great);time+=12000;sea.tick();assert(raw.great);assert.equal(raw.arms.length,24);assert.equal(audit.length,3);
...
console.log('PASS staff-only summons, forged flags/coordinates ignored, normal enemy stats, shared immediate visibility, zero cooldown, audit, permission revocation and world cleanup');
