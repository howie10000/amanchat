...
const fixture={...S,sector:(seed,x,y)=>x===1&&y===0?[{id:'pirate',kind:'pirate',name:'Pirate Sailboat',ship:'sailboat',x:266,y:284,a:0,hp:76,maxHp:76,tier:0,attackAt:0,warning:0}]:[]};
...
actual.handle('aman',{action:'sail'});
...
assert.deepEqual(S.sector(5,2,-1),S.sector(5,2,-1));assert.notDeepEqual(S.sector(5,2,-1),S.sector(6,2,-1));
...
l('interact');assert.equal(l('status').voyage.onFoot,null);const delivered=l('return');assert(delivered.ended);assert(users.aman.sea.deliveries>=2);
