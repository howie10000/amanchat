...
module.exports=function(S){
 ...
 function tick(r,v,entities,time,dt,allies=[]){
  sync(r,v);const mission=entities.find(e=>e.id===r.mission),active=(r.links||[]).some(l=>l.id===r.mission&&S.linkActive(l,time))&&mission&&(mission.kind==='island'||mission.hp>0)&&d(mission,v)<(mission.r||0)+400;
  ...
  for(const h of r.helpers){
   ...
   const move=(goal,radius=3)=>{
    if(d(h,goal)<=radius)return true;
   ...
   if(h.kind==='fighter'&&!living.length){home();continue;}
   h.state='deployed';
   ...
   if(h.kind==='fighter'){
    ...
    if(mission.kind==='island'){if(cache&&!cache.taken){S.takeCache(mission,cache);const tier=cache.tier||0;h.load.push({rarity:['Weathered','Ironbound','Runed','Abyssal'][tier],coins:350+tier*450,gems:3+tier*5,rep:12+tier*10});}}else while(h.load.length<h.capacity&&mission.cargo?.length)h.load.push(mission.cargo.pop());h.state='returning';h.lootTarget=null;
   }
  }
 }
